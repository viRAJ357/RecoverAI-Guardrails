...
# ---------------------------------------------------------------------------
try:
    from razorpay import verify_webhook_signature, payment_link_request, RazorpayClient, EventReceiptStore
...
# ---------------------------------------------------------------------------
# Repo 1 Integration: Razorpay Webhook & Payment Links
# ---------------------------------------------------------------------------
...
async def razorpay_webhook(payload: dict):
    """
    Receives events from Razorpay (e.g. payment.failed), verifies signature,
    and runs the event through the RecoverAI pipeline.
    """
    if not RAZORPAY_CONFIGURED:
         raise HTTPException(status_code=501, detail="Razorpay integration not configured.")
    
    logger.info("Received Razorpay webhook payload: %s", payload)
    
    # Process through our engine
    # In a real scenario, map payload to PaymentEvent
    event = PaymentEvent(
        transaction_id=payload.get("payload", {}).get("payment", {}).get("entity", {}).get("id", "txn_auto"),
        ...
        day_of_week=3,
        is_weekend=0,
        time_since_last_failure_hr=24.0,
